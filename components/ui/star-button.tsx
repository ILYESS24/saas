...
export const StarButton = React.forwardRef<HTMLButtonElement, StarButtonProps>(
  ({
    ...
    useEffect(() => {
      const element = (ref as React.RefObject<HTMLButtonElement>)?.current || internalRef.current;
      if (element) {
        element.style.setProperty(
          "--path",
          `path('M 0 0 H ${element.offsetWidth} V ${element.offsetHeight} H 0 V 0')`,
        );
      }
    }, [ref]);
    ...
    if (asChild && React.isValidElement(children)) {
      ...
      return React.cloneElement(child, {
        ...
        ref: ref,
        children: getButtonContent(childContent),
      });
    }
